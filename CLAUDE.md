...
- **Skills and experiences are merged across the two sources, not ranked.** `pickSkills` and `pickExperiences` (`src/ats/profileHelpers.ts`) concatenate the default resume's `parsedData` with the curated `Profile.skills` / `identity.experiences`, resume entries first, then dedupe. Curated-wins used to be the rule, which meant a hand-built profile could never be updated by uploading a new resume — the parsed list was permanently shadowed. Merging keeps both properties: swapping the default resume changes what gets filled, and nothing hand-entered is ever dropped.
  - Experiences dedupe via `sameRole`: normalized company must match **exactly**, and titles are compared only when both sides have one. Prefix matching would catch `"World Wide Technology"` vs `"World Wide Technology, Inc."` but would also fold `"Google Cloud"` into `"Google"`. A missed match costs a visible duplicate row the user deletes; a wrong match loses a real job with no sign on the form — so the visible failure is chosen deliberately.
  - On a match the parsed values win (that's the resume being attached), but `backfill` fills any field the parse left blank from the curated twin — a regex parse routinely misses a `location` and mangles a `description`.
- `pickProjects` still follows the old curated-wins-then-fall-back rule; nothing parses projects out of a resume yet. **`identity.projects` is the canonical home for projects** — they used to exist only under `resumes[].parsedData.projects`, so a profile with no uploaded resume had nowhere to put them at all.
- **Parsed data is opt-in per resume.** `ResumeManager` extracts on upload but holds the result in component state and renders it for confirmation; only "Use these" calls `setResumeParsedData` and makes it reachable by autofill ("Don't use" on a listed resume clears it again). The merge means parsed fields get typed onto a live application, and `extractExperiences` is regex heuristics over a layout built for human eyes — wrong often enough that it must not take effect unreviewed. `setResumeParsedData` deliberately bypasses `updateProfile`: its deep merge unions arrays element-wise, so a shortened or cleared parsed list could never be written.
- `extractExperiences` (`src/resume/extractExperiences.ts`) anchors every entry on a **date range**, never a lone year — a bare `2025` appears in graduation lines, project years and copyright notices, while `May 2025 – Aug 2025` on a line is almost always an entry header. It handles the single-line (`Company — Title   Dates`) and title-underneath layouts, borrowing the line above only when the date line held nothing else and the line below only when it looks like a role, so a description's first sentence is never swallowed as a title. Company-vs-title is the one real guess: a role keyword wins the title slot, and with no signal the first fragment is taken as the company. An entry with neither a company nor a title is dropped rather than invented.
